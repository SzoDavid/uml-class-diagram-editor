...
export class ConnectionRenderer {
    ...
    public render(node: Connection) {
        ...
        if (node.parts[0].startPoint.isSelected) {
            this._nr.ctx.beginPath();
            this._nr.ctx.arc(node.parts[0].startPoint.x, node.parts[0].startPoint.y, this._nr.rc.dotSize, 0, 2 * Math.PI);
        ...
        node.parts.forEach(part => {
            if (part.endPoint.isSelected) {
                this._nr.ctx.beginPath();
                this._nr.ctx.arc(part.endPoint.x, part.endPoint.y, this._nr.rc.dotSize, 0, 2 * Math.PI);
                this._nr.ctx.fillStyle = this._nr.rc.accentColorSelected;
                this._nr.ctx.fill();
            }
        });
