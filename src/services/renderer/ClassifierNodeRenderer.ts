...
export class ClassifierNodeRenderer {
    ...
    public render(node: ClassifierNode): void {
        const abstractKeyword =
            this._nr.rc.options.classAbstractionDisplayMode ===
            ClassAbstractionDisplayMode.KEYWORD;
        ...
        this._nr.drawHeader(
            ...
            node.name,
            node.header,
            abstractKeyword && node instanceof ClassNode && node.isAbstract
                ? '{abstract}'
                : '',
            node.isSelected,
            invalid,
        ...
        node.height +=
            ...
            this.renderFeatureGroup(
                ...
                node.isSelected,
                invalid,
                node.isNotShownPropertiesExist,
            );
        ...
        node.height +=
            ...
            this.renderFeatureGroup(
                ...
                node.isSelected,
                invalid,
                node.isNotShownOperationsExist,
            );
    }
    ...
    private adjustWidth(node: ClassifierNode): void {
        ...
        node.width = this._nr.roundUpToGridSize(
            Math.max(
                node.width,
                this._nr.ctx.measureText(node.name).width +
                    2 * this._nr.rc.lineMargin,
                node.header
                    ? this._nr.ctx.measureText(`«${node.header}»`).width +
                          2 * this._nr.rc.lineMargin
                    : 0,
                this._nr.rc.options.classAbstractionDisplayMode ===
                    ClassAbstractionDisplayMode.KEYWORD &&
                    ...
                    node.isAbstract
                    ? this._nr.ctx.measureText('{abstract}').width +
                          2 * this._nr.rc.lineMargin
                    : 0,
            ),
        );
        ...
        [...node.properties, ...node.operations].forEach(
            (feature) =>
                (node.width = Math.max(
                    node.width,
                    this._nr.roundUpToGridSize(
                        this.calculateFeatureWidth(feature),
                    ),
                )),
        );
    }
    ...
    private calculateFeatureWidth(feature: Feature): number {
        const baseWidth =
            this._nr.ctx.measureText(feature.toString()).width +
            2 * this._nr.rc.lineMargin;

        if (
            IsMultilineFeature(feature) &&
            feature.toMultilineString().length > 2 &&
            baseWidth >= this._nr.rc.separateObjectParametersWidthLimit
        ) {
            return feature.toMultilineString().reduce((maxWidth, line) => {
                const lineWidth =
                    this._nr.ctx.measureText(line.text).width +
                    2 * this._nr.rc.lineMargin +
                    (line.tabbed ? this._nr.rc.tabSize : 0);
    ...
    private renderFeatures(
        ...
        features.forEach((feature, i) => {
            if (
                ...
                feature.toMultilineString().length <= 2 ||
                this._nr.ctx.measureText(feature.toString()).width <
                    this._nr.rc.separateObjectParametersWidthLimit
            ) {
                this._nr.drawText(
                    feature.toString(),
                    x,
                    ...
                    width,
                    {
                        ...
                        isInvalid: isInvalid,
                        puc:
                            IsDecoratedFeature(feature) &&
                            feature.decorator === 'underline'
                                ? {
                                      prefix: feature.prefix,
                                      underlined: feature.text,
                                  }
                                : null,
                    },
                );
