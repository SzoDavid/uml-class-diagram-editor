...
export class NodeRenderer {
    ...
    public getColor(
        ...
        if (isAccent) {
            if (isSelected)
                return isInvalid
                    ? this.rc.accentColorInvalidSelected
                    : this.rc.accentColorSelected;
            return isInvalid ? this.rc.accentColorInvalid : this.rc.accentColor;
        }
        if (isSelected)
            return isInvalid
                ? this.rc.fillColorInvalidSelected
                : this.rc.fillColorSelected;
    ...
    drawHeader(
        ...
        this.drawRect(
            ...
            y,
            width,
            this.rc.lineHeight * ((header ? 2 : 1) + (footer ? 1 : 0)),
            isSelected,
            isInvalid,
        );
        ...
        if (footer) {
            this.drawText(
                footer,
                x,
                y + this.rc.lineHeight * (header ? 2 : 1),
                width,
                {
                    isSelected: isSelected,
                    isInvalid: isInvalid,
    ...
    drawText(
        ...
        if (width !== -1) {
            switch (textAlign) {
                ...
                case 'left':
                    xDelta =
                        this.rc.lineMargin + (isTabbed ? this.rc.tabSize : 0);
                    wDelta = -2 * this.rc.lineMargin;
                    break;
            }
        }
        ...
        this.ctx.fillText(
            text,
            x + xDelta,
            y + this.rc.lineHeight / 2,
            width !== -1 ? width + wDelta : undefined,
        );
        ...
        if (puc) {
            ...
            this.ctx.moveTo(
                x + xDelta + uDelta + this.ctx.measureText(puc.prefix).width,
                y +
                    this.rc.lineHeight / 2 +
                    this.rc.textSize / 2 +
                    this.rc.underlineDelta,
            );
            this.ctx.lineTo(
                x +
                    xDelta +
                    uDelta +
                    this.ctx.measureText(puc.prefix).width +
                    this.ctx.measureText(puc.underlined).width,
                y +
                    this.rc.lineHeight / 2 +
                    this.rc.textSize / 2 +
                    this.rc.underlineDelta,
    ...
    roundUpToGridSize(value: number): number {
        return (
            Math.ceil(value / (this.rc.options.gridSize * 2)) *
            (this.rc.options.gridSize * 2)
        );
    }
    ...
    private renderPrimitiveType(node: PrimitiveTypeNode) {
        ...
        node.width = this.roundUpToGridSize(
            Math.max(
                this.rc.defaultWidth,
                this.ctx.measureText(node.name).width + 2 * this.rc.lineMargin,
                this.ctx.measureText('«Primitive»').width +
                    2 * this.rc.lineMargin,
            ),
        );
    ...
    private renderEnumeration(node: EnumerationNode) {
        ...
        node.width = this.roundUpToGridSize(
            Math.max(
                this.rc.defaultWidth,
                this.ctx.measureText(node.name).width + 2 * this.rc.lineMargin,
                this.ctx.measureText('«Enumeration»').width +
                    2 * this.rc.lineMargin,
            ),
        );
        ...
        node.values.forEach((value) => {
            node.width = Math.max(
                node.width,
                this.ctx.measureText(value).width + 2 * this.rc.lineMargin,
            );
        });
        ...
        this.drawRect(
            ...
            node.y + node.height,
            node.width,
            node.values.length * this.rc.lineHeight,
            node.isSelected,
            invalid,
        );
