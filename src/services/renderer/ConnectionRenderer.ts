...
export class ConnectionRenderer {
    ...
    public render(node: Connection): void {
        ...
        for (let i = 0; i < node.parts.length - 1; i++) {
            ...
            this._nr.ctx.strokeStyle = this._nr.getColor(true, node.isSelected || part.isSelected, invalid);
            this._nr.ctx.stroke();
        ...
        this._nr.ctx.strokeStyle = this._nr.getColor(true, node.isSelected || node.parts[node.parts.length - 1].isSelected, invalid);
        this._nr.ctx.stroke();
    ...
    private renderPoint(point: Point, isSelected: boolean, isInvalid: boolean): void {
        this._nr.ctx.beginPath();
    ...
    private renderEndDecorations(connection: Connection, startPart: ConnectionPart, endPart: ConnectionPart, isInvalid: boolean): void {
        if (connection instanceof Association) {
            ...
            this.handleGeneralization(connection, startPart, endPart, isInvalid);
        }
    }

    private handleAggregation(connection: Aggregation, startPart: ConnectionPart, endPart: ConnectionPart, isInvalid: boolean): void {
    ...
    private handleAssociation(connection: Association, startPart: ConnectionPart, endPart: ConnectionPart, isInvalid: boolean): void {
        ...
        if (connection.parts.length % 2 === 1) {
            ...
            if ((angle  > Math.PI / 4 && angle < 3 * Math.PI / 4) || (angle > 5 * Math.PI / 4 && angle < 7 * Math.PI / 4)) {
                this._nr.drawText(
                    ...
                    -1,
                    {isSelected: isSelected, textAlign: 'left', isInvalid: isInvalid}
                );
            } else {
                this._nr.drawText(
                    connection.associationName,
                    ...
                    -1,
                    {isSelected: isSelected, textAlign: 'center', isInvalid: isInvalid}
                );
            }
        } else {
            const midPart = connection.parts[Math.floor(connection.parts.length / 2)];
    ...
    private handleComposition(connection: Composition, startPart: ConnectionPart, endPart: ConnectionPart, isInvalid: boolean): void {
    ...
    private handleGeneralization(connection: Generalization, startPart: ConnectionPart, endPart: ConnectionPart, isInvalid: boolean): void {
        if (connection.reversed) {
    ...
    private renderFilledTriangle(point: Point, angle: number, isSelected: boolean = false, isInvalid: boolean = false): void {
        ...
        this._nr.ctx.fillStyle = this._nr.getColor(false, isSelected, isInvalid);
        this._nr.ctx.fill();

        this._nr.ctx.strokeStyle = this._nr.getColor(true, isSelected, isInvalid);
        this._nr.ctx.stroke();
    }
    ...
    private renderDiamond(point: Point, angle: number, isSelected: boolean = false, isFilled: boolean = false, isInvalid = false): void {
        ...
        this._nr.ctx.fillStyle = this._nr.getColor(isFilled, isSelected, isInvalid);
        this._nr.ctx.fill();
    ...
    private renderEndTexts(connection: Composition|Aggregation|Association, startPart: ConnectionPart, endPart: ConnectionPart, isInvalid: boolean): void {
        this.renderEndText(
            startPart.startPoint,
    ...
    private renderEndText(point: Point, offset: PixelOffset, textA: string, textB: string, angle: number, isSelected: boolean = false, isInvalid: boolean = false): void {
        ...
        if (point instanceof LooseConnectionPoint) {
            if (point.x === point.node.x || point.x === point.node.x + point.node.width) {
                this.renderEndTextVertically(coords, 40, textA, textB, isSelected, angle < Math.PI / 2 || angle > 3 * Math.PI / 2, isInvalid);
            } else {
                this.renderEndTextHorizontally(coords, 40, textA, textB, isSelected, isInvalid);
            }
            return;
        }
    ...
    private renderEndTextVertically(point: Point, distance: number, topText: string, bottomText: string, isSelected: boolean, alignLeft: boolean, isInvalid: boolean): void {
        if (topText) this._nr.drawText(topText, point.x, point.y - (this._nr.rc.lineHeight / 2) + (distance / 2), -1, {isSelected: isSelected, textAlign: alignLeft ? 'left' : 'right', isInvalid: isInvalid});
