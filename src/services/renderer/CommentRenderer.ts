...
export class CommentRenderer {
    ...
    public render(node: CommentNode): void {
        ...
        lines.forEach((line, index) => {
            this._nr.drawText(
                line,
                node.x,
                node.y + index * this._nr.rc.lineHeight,
    ...
    private getLines(text: string, maxWidth: number): string[] {
        ...
        words.forEach((word) => {
            ...
            if (
                this._nr.ctx.measureText(currentLine + word).width <
                maxWidth - (lines.length === 0 ? this._nr.rc.lineHeight / 2 : 0)
            ) {
                currentLine += word + ' ';
            } else {
                lines.push(currentLine.trim());
    ...
    private drawCommentBox(
        ...
        const strokeStyle = isSelected
            ? isInvalid
                ? this._nr.rc.accentColorInvalidSelected
                : this._nr.rc.accentColorSelected
            : isInvalid
              ? this._nr.rc.accentColorInvalid
              : this._nr.rc.accentColor;
        ...
        this._nr.ctx.fillStyle = isSelected
            ? isInvalid
                ? this._nr.rc.fillColorInvalidSelected
                : this._nr.rc.fillColorSelected
            : isInvalid
              ? this._nr.rc.fillColorInvalid
              : this._nr.rc.fillColor;
        this._nr.ctx.fill();
