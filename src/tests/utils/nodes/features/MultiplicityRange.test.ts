...
describe('UCDE-MultiplicityRange', () => {
    ...
    describe('UCDE-MR-0200-validate', () => {
        ...
        describe('UCDE-MR-0202 GIVEN invalid upper WHEN validate() THEN return excepted valid error', () => {
            test.each([
                { upper: 0, expectedErrors: [{parameter: 'upper', message: 'error.multiplicity_range.upper_not_larger_than_zero'}] },
                { upper: -1, expectedErrors: [{parameter: 'upper', message: 'error.multiplicity_range.upper_not_larger_than_zero'}] },
                { upper: 'invalid', expectedErrors: [{parameter: 'upper', message: 'error.multiplicity_range.invalid_non_numeric_value'}] }
        ...
        describe('UCDE-MR-0203 GIVEN invalid lower WHEN validate() THEN return expected valid error', () => {
            test.each([
                {upper: 5, lower: -1, expectedErrors: [{parameter: 'lower', message: 'error.multiplicity_range.lower_less_than_zero'}] },
                {upper: 5, lower: 5, expectedErrors: [{ parameter: 'lower', message: 'error.multiplicity_range.lower_not_less_than_upper' }] },
                {upper: 5, lower: 6, expectedErrors: [{ parameter: 'lower', message: 'error.multiplicity_range.lower_not_less_than_upper' }] },
