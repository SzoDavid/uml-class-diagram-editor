...
function exportToImage() {
    ...
    const nodes = SerializationRegistryService.batchDeserialize<Node>(JSON.parse(save));
    ...
    for (const node of nodes) {
        if (node instanceof PositionalNode) {
            ...
            if (node.x + node.width > rightEdge) rightEdge = node.x + node.width;
            if (node.y + node.height > bottomEdge) bottomEdge = node.y + node.height;
    ...
    canvas.setAttribute('width', String(rightEdge - leftEdge + 2 * imageMarginSize.value));
    canvas.setAttribute('height', String(bottomEdge - topEdge + 2 * imageMarginSize.value));

    const editor = new UmlEditorService(canvas, new Renderer(canvas, settings.renderer), true);
    editor.nodes = nodes;
...
<template>
  <v-card :title="t('export_document')">
    <v-card-text>
      <v-select :label="t('export_method')" v-model="exportMethod" density="comfortable" :items="[
        { title: 'PNG', value: 'png' }
      ]" />
    ...
    <v-card-actions>
      ...
      <v-btn @click="exportDiagram" color="primary">{{ t('export_document') }}</v-btn>
    </v-card-actions>
  </v-card>
</template>
