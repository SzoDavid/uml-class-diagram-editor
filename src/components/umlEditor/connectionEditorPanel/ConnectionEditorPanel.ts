...
interface ConnectionEditorPanelEmits {
    (
        e: 'save',
        data: NodeData<Connection | ConnectionPart | ConnectionPoint>,
    ): void;
    (e: 'render'): void;
}
...
export default defineComponent({
    ...
    props: {
        connectionData: {
            type: Object as () => NodeData<
                Connection | ConnectionPart | ConnectionPoint
            >,
            required: true,
        },
    },
    emits: ['save', 'render'],
    setup(
        props: ConnectionEditorPanelProperties,
        { emit }: { emit: ConnectionEditorPanelEmits },
    ) {
        const { t } = useI18n();

        const data = ref<
            NodeData<Connection | ConnectionPart | ConnectionPoint>
        ...
        const breakConnectionPart = () => {
            if (
                data.value === null ||
                !(data.value.instance instanceof ConnectionPart)
            ) {
                console.error(
                    'break connection part is called for invalid node',
                    data.value,
                );
                return;
            }
        ...
        const getError = (context: ErrorContext) => {
            if (data.value === null || data.value.type !== 'connection')
                return null;
